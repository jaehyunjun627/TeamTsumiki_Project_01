...
<body>
	<div class="container">
		<div class="header">
			<a href="<%=contextPath%>/groupSelect?level=<%=level%>"
				class="back-link">그룹 선택</a>
			<div class="group-info">
				그룹
				<%=String.format("%02d", groupNumber)%></div>
			<div class="progress">
		...
		<div class="kanji-card">
			<div class="kanji-korean">
				<%=currentKanji.getExplanation() != null ? currentKanji.getExplanation() : ""%>
			</div>
			...
			<div class="kanji-readings">
				<div class="reading-item">
					<span class="reading-label">음독 : </span> <span
						class="reading-value"><%=onyomi.isEmpty() ? "-" : onyomi%></span>
				</div>

				<div class="reading-item">
					<span class="reading-label">훈독 : </span> <span
						class="reading-value"><%=kunyomi.isEmpty() ? "-" : kunyomi%></span>
				</div>
			</div>
		...
		<div class="button-section">
			<button class="action-button finish-button"
				onclick="location.href='<%=contextPath%>/finishGroup?level=<%=level%>&group=<%=groupNumber%>'">
				학습 완료</button>
			<button class="action-button home-button"
				onclick="location.href='<%=contextPath%>/groupSelect?level=<%=level%>'">
				그룹 선택으로</button>
		</div>
	</div>
</body>
